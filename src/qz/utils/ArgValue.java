...
public enum ArgValue {
    ...
    SECURITY_WSS_HOST(PREFERENCES, "Influences which physical adapter to bind to by setting the host parameter for http/websocket listening", null, "127.0.0.1",
                      "security.wss.host"),
    SECURITY_WSS_ALLOWORIGIN(PREFERENCES, "Override 'Access-Control-Allow-Origin: *' HTTP response header for fine-grained control of incoming HTTP connections.  Multiple values may be separated by commas but will only apply to WebSocket connections.", null, "*",
